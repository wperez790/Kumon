...
public class AlumnoBO {
    ...
    public boolean registrar(Alumno alumno) throws Exception {
        ...
        if (!personaBO.registrar(alumno)) {
            ok = false;
        }
        if (!alumnoDB.registrar(alumno)) {
            ok = false;
        }
        if (!generarDeuda(alumno)) {
            ok = false;
        }
        if (!asignaturaBO.registrar(alumno.getListaAsignaturas(), alumno.getIdAlumno())) {
            ok = false;
        }
        return ok;
    ...
    void eliminar(String idPersona) throws Exception {
        alumnoDB.eliminar(idPersona);
    }
